...
if __name__ == "__main__":
  ...
  if "1" in arg:
    ...
    #pred =  [float(x.strip().split()[1]) for x in rawPred.strip().split('\n')]
    #ns =  [float(x.strip().split()[0]) for x in rawPred.strip().split('\n')]
    #print len(ns), len(blackGaus)
    rns = [1-x for x in ns]
    plt.plot(ns,a, 'g-', ns,b, 'r-', ns, rns, 'b-', ns, c, 'm-', ns, d, 'c-')
    plt.annotate("Prim 5*5", (ns[20],a[20]), xytext = (0.2, 10), textcoords = 'offset points', ha = 'right', va = 'bottom', bbox = dict(boxstyle = 'round,pad=0.2', fc = 'yellow', alpha = 0.3), arrowprops = dict(arrowstyle = '-', connectionstyle = 'arc3,rad=0'))
    plt.annotate("Kruskal 5*5", (ns[60],b[60]), xytext = (40, 20), textcoords = 'offset points', ha = 'right', va = 'bottom', bbox = dict(boxstyle = 'round,pad=0.2', fc = 'yellow', alpha = 0.3), arrowprops = dict(arrowstyle = '-', connectionstyle = 'arc3,rad=0'))
    plt.annotate("No cascading", (ns[30],rns[30]), xytext = (50, 20), textcoords = 'offset points', ha = 'right', va = 'bottom', bbox = dict(boxstyle = 'round,pad=0.2', fc = 'yellow', alpha = 0.3), arrowprops = dict(arrowstyle = '-', connectionstyle = 'arc3,rad=0'))
